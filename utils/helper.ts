...
export class listedItemResponse {
  ...
  isExist: boolean
  marketItemEntity: itemListedEntity
  constructor(_isExist: boolean, _marketItemEntity: itemListedEntity) {
    this.isExist = _isExist;
    this.marketItemEntity = _marketItemEntity
  }
}
export function getMarketItem(id: string): listedItemResponse {
  let isExist = true;
